...
#include <tomato/ImGui/Modules/ExternalConsoleSink.h>
...
namespace tomato
{
	...
	void ConsolePanel::ImGuiRenderHeader()
	{
        ...
        float levelButtonWidth = (ImGui::CalcTextSize(Message::GetLevelIcon(Log::Level::Trace)) + ImGui::GetStyle().FramePadding * 2.0f).x;
		...
		{
			ImGui::SameLine();
			auto level = static_cast<Log::Level>(std::pow(2, i));

			bool levelEnabled = s_MessageBufferRenderFilter & level;
			Vec4 c = Message::GetRenderColor(level);
			if(levelEnabled)
            {
	...
	void ConsolePanel::Message::OnImGuiRender() const
	{
		...
		Vec4 c = GetRenderColor(Level);
		ImGui::PushStyleColor(ImGuiCol_Text, c);
		ImGui::PushFont(EditorTheme::BoldFont);
		auto levelIcon = GetLevelIcon(Level);
		ImGui::TreeNodeEx(&ID, flags, "%s  %s", levelIcon, Buffer.c_str());
		ImGui::PopFont();
		ImGui::PopStyleColor();
	...
	Vec4 ConsolePanel::Message::GetRenderColor(Log::Level level)
	{
		switch (level)
		{
