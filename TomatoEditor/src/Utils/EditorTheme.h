...
namespace tomato
{
	class EditorTheme
	{
		static ImVec4 Darken(ImVec4 c, float p) { return { std::max(0.f, c.x - 1.0f * p), std::max(0.f, c.y - 1.0f * p), std::max(0.f, c.z - 1.0f * p), c.w }; }
		static ImVec4 Lighten(ImVec4 c, float p) { return { std::max(0.f, c.x + 1.0f * p), std::max(0.f, c.y + 1.0f * p), std::max(0.f, c.z + 1.0f * p), c.w }; }
