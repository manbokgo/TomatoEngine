...
namespace tomato
{
    ...
    {
        // m_Mesh = CResMgr::GetInst()->FindRes<Mesh>(L"RectMesh");
        m_Material = CResMgr::GetInst()->FindRes<Material>(L"TileMapMtrl");
    ...
    void TileMapComponent::render()
    {
        ...
        GetMaterial()->SetScalarParam(eScalarParam::Vec2_0, &vLeftTop);
        GetMaterial()->SetScalarParam(eScalarParam::Vec2_1, &vSlice);
        GetMaterial()->SetScalarParam(eScalarParam::Vec2_2, &m_vTileCount);
    
        GetMaterial()->SetTexParam(eTexParam::Tex0, m_AtlasTex);
